...
description: >-
   프론트에서 api 를 반복적으로 호출하여 서버의 response 를 확인하는 방법은 무엇일까?
author: Lois
date: 2023-01-05
...
## Intro,
서버에서 비동기 작업 처리에 시간이 걸리는 관계로, 프론트에서 api post 이후 해당 작업을 백엔드에서 처리(done) 되었는지 확인해주어야 하는 작업을 구현해주어야 했다. 즉, 백엔드에서 작업을 처리했는지 여부를 확인하는 api 를 프론트에서 반복적으로 호출하여 response 를 받아야 하는데 이 개념은 순수 JavaScript 로 구현을 할 수는 있지만, 코드가 상당히 복잡해지는 문제가 있어서 RxJS 를 적용해보기로 했다.

- step1: API POST 요청 이후, 응답으로 Task id 를 받는다.
- step2: 서버의 비동기 작업이 끝났는지 여부를 알 수 있는 GET API 를 활용하여 Task id 를 기반으로 주기적으로 요청을 보낸다.
...
**polling 은 클라이언트에서 서버로 주기적으로 request 를 하는 방법**이다. 시간 간격을 유지하며 요청할 수 있기에 응답 간격 또한 유지할 수 있다. 하지만, 이 방식은 실시간 통신이 필요할 경우 적합한 방식은 아니다. 시간 간격을 줄여서 요청을 계속 보내면 서버 부하가 생길 수 있기 때문이다.
...
(Observable 모델을 사용하면 Array과 같은 간단한 작업과 동일한 방식으로 비동기 이벤트의 스트림을 처리할 수 있다, 출처: https://reactivex.io/intro.html)
> 
...
## RxJS 를 이용한 polling
 ...
 서두에 언급한 것 처럼, 백엔드에서 작업을 처리했는지 여부를 주기적으로 확인하는 방법을 구현해야 했다. 백엔드 api 인 `task()` 는 작업이 완료되지 않았다면 `null`, 완료 되었다면 `done` 을 바로 반환한다. 요구 사항은 요약하자면 아래와 같다. 

### Flow

[1] 모달 안의 특정 버튼을 클릭한다. 

[2] 해당 버튼을 클릭하면, 특정 api 를 호출하고 task 가 완료되었는지 확인할 수 있는 `taskId` 를 반환한다.

[3] `taskId` 를 이용하여 `task()` post api를 호출 후, `done` 이 되었는지 확인한다. 

- 설정 시간동안 api 호출이 로딩 상태라면, 모달 내 버튼을 disable 처리한다.
- time out 되었거나 응답으로 `done` 을 반환했다면 모달을 닫아준다.

우선 나는 해당 task를 확인하는 작업이 다른 여러 api 에서 사용해야했기 때문에, `useTaskGet` 이라는 공통 hook 으로 만들어주었다. `useTaskGet` hook 에서는 `taskId` 와 polling 이 성공했을 시 실행하는 `onSuccess` 콜백, 실패했을 시 실행하는 `onFail` 함수를 props 로 받는다. 그리고 hook 의 리턴 값으로 `taskStatus (’done’ 또는 ‘null’)` 을 반환한다. 예를 들어 onSuccess 로직으로는 polling 을 성공한 후 alert 을 띄우거나 modal 을 닫는 경우를 고려한다. 
...
아래 예제는 Next.js 의 조합으로 만들어두었다.

[](https://github.com/Rory0304/polling-example/blob/main/client/src/hooks/useTaskGet.ts)
